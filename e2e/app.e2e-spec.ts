...
describe('Unlock Your Device', () => {
  it('Should open device unlock page', () => {
    ...
    expect(browser.getCurrentUrl()).toEqual(
      'http://localhost:4200/#/device-unlock'
    );
  });
  ...
  it('Should Validate AT&T Wireless Number', () => {
    element(by.name('wirelessNumber')).sendKeys(1234567890);
    element(by.name('wirelessNumber'))
      .getAttribute('value')
      .then(function(text) {
  ...
  it('Should Reach Personal Info Page', () => {
    ...
    expect(browser.getCurrentUrl()).toEqual(
      'http://localhost:4200/#/unlockstep2;wirelessNumber=1234567890'
    );
  });

  it('Should Fill Account Holders First Name', () => {
    element(by.id('first_name')).sendKeys('Michell');
    expect(element(by.id('first_name')).getAttribute('value')).toEqual(
      'Michell'
    );
  });
  ...
  it('Should Fill Account Holders Last Name', () => {
    ...
    expect(element(by.id('last_name')).getAttribute('value')).toEqual(
      'Johnson'
    );
  });
  ...
  it('Should Fill Account Holders passcode', () => {
    ...
    expect(element(by.id('passcode')).getAttribute('value')).toEqual(
      '12345678'
    );
  });
  ...
  it('Should Fill Account Holders email id', () => {
    ...
    expect(element(by.id('email')).getAttribute('value')).toEqual(
      'admin@att.com'
    );
  });
  ...
  it('Should Fill Account Holders email id again', () => {
    ...
    expect(element(by.id('email_again')).getAttribute('value')).toEqual(
      'admin@att.com'
    );
  });
  ...
  it('Should Check both email address are same', () => {
    expect(element(by.id('email')).getAttribute('value')).toEqual(
      element(by.id('email_again')).getAttribute('value')
    );
  });
  ...
  it('Should Redirect to unlockstep3 page', () => {
    ...
    expect(browser.getCurrentUrl()).toEqual(
      'http://localhost:4200/#/unlockstep3'
    );
  });
  ...
  it('Should Submit the Order', () => {
    ...
    expect(browser.getCurrentUrl()).toEqual(
      'http://localhost:4200/#/unlockConfirm;customerType=true'
    );
  });
});
...
describe('Check an Unlock status', () => {
  it('Should open unlock status page', () => {
    ...
    expect(browser.getCurrentUrl()).toEqual(
      'http://localhost:4200/#/unlock-status'
    );
  });
  ...
  it('Should reach current status page', () => {
    ...
    expect(browser.getCurrentUrl()).toEqual(
      'http://localhost:4200/#/unlock-status-confirm;imeiNumber=322765434587853'
    );
  });
});
